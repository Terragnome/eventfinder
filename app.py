...
from helpers.jinja_helper import relpath, round_ct, pluralize, filter_url_params, update_url_params, strip_url, strip_phone
...
def _parse_chips(
  ...
  if flags is None:
    ...
    flags = set(x.strip() for x in flags.split(","))
...
@oauth2_required
def users(
  query=None, tag=None, flags=None,
  page=1, next_page_url=None, prev_page_url=None,
  scroll=False, selected=None
):
  current_user = UserController().current_user
